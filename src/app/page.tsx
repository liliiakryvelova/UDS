      ...
      <section className="mt-8 space-y-8">
        {filteredGroups.map((group) => (
          <div key={group.slug}>
            ...
            <div className="mt-4 grid auto-rows-fr items-stretch justify-items-center gap-4 md:grid-cols-2 xl:grid-cols-3">
              {group.events.length > 0 ? (
                group.events.map((event) => (
                  <Link
                    key={event.id}
                    href={`/c/${group.slug}/events/${event.id}`}
                    className="group flex min-h-[22rem] w-full max-w-sm self-stretch flex-col overflow-hidden rounded-2xl border border-sky-200 bg-sky-50 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
                  >
                    ...
                    <div className="flex flex-1 flex-col p-4">
                      ...
                      <h3 className="mt-1 text-lg font-semibold text-slate-900">{event.name}</h3>
                      <p className="mt-2 line-clamp-2 text-sm text-slate-700">{event.shortDescription}</p>
                      <p className="mt-3 text-xs text-slate-600">
                        {event.startDate} - {event.endDate} | {event.venueName}
                      </p>
                      <span className="mt-auto inline-flex self-center rounded-full bg-sky-800 px-4 py-2 text-sm font-medium text-white shadow-sm transition group-hover:bg-sky-900">
                        Open Event
                      </span>
                    </div>
                  </Link>
                ...
                <div className="w-full rounded-2xl border border-dashed border-sky-300 bg-sky-50 p-5 text-sm text-slate-600 md:col-span-2 xl:col-span-3">
